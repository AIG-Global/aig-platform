...
// functions and AIGAuth.getCurrentUser().
//
...
(function () {
  ...
  const USERS_KEY = "aig_auth_users_v1";
  const SESSION_KEY = "aig_auth_session_v1";
  const RISK_DISCLOSURE_VERSION = "2026-06-30"; // bump this if the risk text materially changes
  ...
  /**
   * Register a new account. Throws on any validation failure, including
   * a missing risk acknowledgment — registration is REQUIRED to include
   * this, the same way a real backend should reject a registration
   * payload that doesn't include it.
   *
   * @param {{email: string, password: string, riskAcknowledged: boolean}} params
   * @returns {Promise<{email: string}>}
   */
  async function register({ email, password, riskAcknowledged }) {
    const normalizedEmail = normalizeEmail(email);
    if (!validateEmail(normalizedEmail)) throw new Error("Enter a valid email address.");
    if (!password || password.length < 8) throw new Error("Password must be at least 8 characters.");
    if (!riskAcknowledged) throw new Error("You must acknowledge the investment risk disclosure to create an account.");
    ...
    users[normalizedEmail] = {
      ...
      riskDisclosureVersion: RISK_DISCLOSURE_VERSION
    };
    saveUsers(users);
